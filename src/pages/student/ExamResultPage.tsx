...
const mockResult = {
  ...
  questionResults: [
    { question: 'What is the time complexity of binary search?', earned: 2, max: 2, correct: true },
    { question: 'Which of the following are programming paradigms?', earned: 2, max: 3, correct: false },
    { question: 'What does CPU stand for?', earned: 1, max: 1, correct: true },
    { question: 'Explain the concept of recursion', earned: 3, max: 5, correct: true },
  ],
};

const mockLeaderboard = [
  { rank: 1, name: 'Alice Johnson', score: 11, submitted_at: '2025-11-14T09:45:00' },
  { rank: 2, name: 'Bob Smith', score: 10, submitted_at: '2025-11-14T10:12:00' },
  { rank: 3, name: 'Carol White', score: 9, submitted_at: '2025-11-14T10:05:00' },
  { rank: 4, name: 'David Brown', score: 9, submitted_at: '2025-11-14T10:20:00' },
  { rank: 5, name: 'You', score: 8, submitted_at: '2025-11-14T10:30:00', isCurrentUser: true },
];
...
export const StudentExamResultPage = () => {
  ...
  const percentage = (mockResult.submission.total_score / mockResult.maxScore) * 100;
    ...
    <Layout>
      <Box>
        <Paper elevation={3} sx={{ p: 4, mb: 3, textAlign: 'center' }}>
          ...
          <Box sx={{ my: 3 }}>
            <Typography variant="h2" color={passed ? 'success.main' : 'error.main'} fontWeight="bold">
          ...
          <Box sx={{ mt: 3, display: 'flex', justifyContent: 'center', gap: 2 }}>
            <Box sx={{ textAlign: 'center' }}>
              <Typography variant="h6" color="primary">{mockResult.rank}</Typography>
              <Typography variant="caption" color="text.secondary">Your Rank</Typography>
            </Box>
            <Divider orientation="vertical" flexItem />
            <Box sx={{ textAlign: 'center' }}>
              <Typography variant="h6" color="primary">{mockResult.totalParticipants}</Typography>
              ...
              <TableBody>
                {mockLeaderboard.map((entry) => (
                  <TableRow 
                    key={entry.rank}
                    sx={{ 
                      bgcolor: entry.isCurrentUser ? 'primary.light' : 'inherit',
